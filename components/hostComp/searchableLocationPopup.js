...
const SearchablePopup = (props) => {
  ...
  const submitHandler = () => {
    if (selectedItems && selectedItems.length > 0 && userID) {
      setIsLoading(true);
      dispatch(userActions.updateLiveLocation(userID, selectedItems[0]))
        .then(() => {
          setIsLoading(false);
              ...
              fetch(placeDetailUrl)
                .then((res) => {
                  res.json().then((queryData) => {
                    ...
                    shortName = shortName.replace("Metropolitan", "");
                    shortName = shortName.replace("Municipality", "");
                    shortName = shortName.replace("Metro", "");
                    shortName = shortName.replace("District", "");
                    shortName = shortName.trim();

                    shortName = shortName.replace(" ", "");
                    var nameLength = shortName.length;
                    shortName =
                      shortName.substring(0, 1) +
                      shortName.substring(
                        Math.round(nameLength / 2),
                        Math.round(nameLength / 2) + 1
                      ) +
                      shortName.substring(nameLength - 1, nameLength);
      ...
      >
        ...
        <View style={styles.resultContainer}>
          ...
          {selectedItems.length >= 1 ? (
            <View style={{ flex: 1 }}>
              <View style={styles.listHeaderContainer}>
                <Text style={styles.subHeaderText}>SELECTED LOCATION:</Text>
              </View>
              <FlatList
                style={{ flex: 1 }}
                data={selectedItems}
